...
username_cmems = 'XXXXXX'
password_cmems = 'XXXXXX'
...
# Define a dict to get required parameters of the data request. 
...
dict_id = {
    "eastward_sea_water_velocity": \
           ["-v utotal", "-d global-analysis-forecast-phy-001-024-hourly-merged-uv", \
          "-s GLOBAL_ANALYSIS_FORECAST_PHY_001_024-TDS", "-m http://nrt.cmems-du.eu/motu-web/Motu"], \
          "northward_sea_water_velocity": \
         ["-v vtotal", "-d global-analysis-forecast-phy-001-024-hourly-merged-uv", \
           "-s GLOBAL_ANALYSIS_FORECAST_PHY_001_024-TDS", "-m http://nrt.cmems-du.eu/motu-web/Motu"]
          }
...
absolute_path_substring = ['/home/', 'E:\\']
...
for key, value in dict_id.items():
    ...
    # Date declaration
    date_start = dt.datetime(yyyystart,mmstart,ddstart,0,0)
    date_end = dt.datetime(yyyyend,mmend,ddend,0,0)
    ...
    while (date_start <= date_end):
        ...
        if not os.path.exists(out_cmd.split()[1] + ficout):
            if proxy_flag:
                if not zmin_depth:
                    cmd = ' '.join([motu_cl, log_cmems, pwd_cmems,                                value[3], value[2], value[1],                                xmin, xmax, ymin, ymax,                                date_cmd, value[0], out_cmd, fic_cmd,                                proxy_server, proxy_user, proxy_pwd, "-q"])
                else:
                    cmd = ' '.join([motu_cl, log_cmems, pwd_cmems,                                value[3], value[2], value[1],                                xmin, xmax, ymin, ymax, zmin, zmax,                                date_cmd, value[0], out_cmd, fic_cmd,                                proxy_server, proxy_user, proxy_pwd, "-q"])
            else:
                if not zmin_depth:
                    cmd = ' '.join([motu_cl, log_cmems, pwd_cmems,                                value[3], value[2], value[1],                                xmin, xmax, ymin, ymax,                                date_cmd, value[0], out_cmd, fic_cmd, "-q"])
                else:
                    cmd = ' '.join([motu_cl, log_cmems, pwd_cmems,                                value[3], value[2], value[1],                                xmin, xmax, ymin, ymax, zmin, zmax,                                date_cmd, value[0], out_cmd, fic_cmd, "-q"])
